...
The `json` driver supports one PatchPage Node process. Within that process, database objects targeting the same backing-directory filesystem identity — including paths through symlinked parents, containing-directory bind-mount aliases, and case aliases on case-insensitive filesystems — share one mutation serializer, even before the final file exists. Each mutation completes its read, state-shape validation, update, and commit before the next mutation starts. This coordination is strictly process-local; the driver does not provide interprocess locking.
