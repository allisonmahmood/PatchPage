...
Each section states its inputs as a table; export those, then run the one documented invocation. Anything this deployment needs that is *not* an `ops.sh` command is deliberately human-in-the-loop and appears here as prose rather than as a block to paste.
